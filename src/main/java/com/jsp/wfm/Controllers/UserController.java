...
import com.jsp.wfm.Exception.RandomCustomException;
import com.jsp.wfm.Models.Entities.BigPojo;
import com.jsp.wfm.Models.Entities.User;
import com.jsp.wfm.Models.Entities.UserListInfo;
import com.jsp.wfm.Models.Entities.pojo;
import com.jsp.wfm.Services.CustomService;
import com.jsp.wfm.Services.TserveI;
...
public class UserController {
    ...
    public Optional<User> getOpUser(String username) {
        Optional<User> optionalUser = userService.getOpUser(username);
        return optionalUser;
    }

    public int implPickupTest() {
        return tserveI.doSomething();
    }

    @SneakyThrows
    public String throwSneakyException(String username) throws Exception {
        User user = userService.getUser(username);
        if (user.getUsername() == null) {
            throw new RandomCustomException("Random text", "Code 0");
        } else {
...
//        return userService.throwExceptionTest();
    ...
    public DateHolder getDatesAndInstants() {
        Instant i = userService.getInstantJavaTime();
        org.joda.time.Instant i2 = userService.getJodaInstant();
        DateTime dt = userService.getJodaDateTime();
        DateHolder dateHolder = new DateHolder(i, i2, dt);
        return dateHolder;
    }

    class DateHolder {
        Instant i;
        org.joda.time.Instant i2;
        DateTime dt;

        public DateHolder(Instant i, org.joda.time.Instant i2, DateTime dt) {
            this.i = i;
            this.i2 = i2;
            this.dt = dt;
        }
